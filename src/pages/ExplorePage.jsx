...
import { Bookmark, Info, Plus } from 'lucide-react'
...
export default function ExplorePage({ navigate }) {
  ...
  return <>
    ...
    <Panel className="market-panel" id="market-results">
      <div className="market-toolbar"><div className="market-toolbar__left"><SearchField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search token or pool" /><SegmentedControl items={['All strategies', 'Dynamic liquidity', 'Range liquidity']} value={model} onChange={setModel} label="Liquidity strategy" /><button className="saved-filter" onClick={() => setView('Saved')}><Bookmark size={17} />Saved {saved.length > 0 && <em>{saved.length}</em>}</button></div><div className="market-toolbar__right"><SegmentedControl items={['TVL', 'Volume', 'Fees', 'APR', 'New']} value={sort} onChange={setSort} label="Sort pools" /><div className="window-select"><span>Window</span><DropdownSelect label="Select data window" value={window} onChange={setWindow} options={['1H', '2H', '24H', '7D']} /></div></div></div>
