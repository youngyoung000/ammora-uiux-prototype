...
export default function CurrenciesPage({ navigate }) {
  const [query, setQuery] = useState('')
  const [group, setGroup] = useState('All currencies')
  const stablecoins = ['USDT', 'USDC', 'DAI', 'GBPM', 'COPM', 'JPYS']
  ...
  return <>
    <WorkspaceHeader title="Token directory" meta={<Badge tone="neutral" className="info-badge"><Info size={16} />Information only</Badge>} />
    <Panel className="currency-panel" id="currency-results"><div className="currency-toolbar"><SearchField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search assets or addresses" /><SegmentedControl items={['All currencies', 'Stablecoins', 'Major assets']} value={group} onChange={setGroup} label="Currency group" /></div><div className="currency-table"><div className="currency-row currency-head"><span>Token</span><span>Chain</span><span>Price</span><span>24h</span><span>24h volume</span><span>Market cap</span><span>Liquidity</span><span>Holders</span><span /></div>{visible.map((item) => <div className="currency-row" key={item.symbol} role="button" tabIndex="0" onClick={() => navigate(`currency/${item.symbol.toLowerCase()}`)}><div className="currency-name"><TokenIcon symbol={item.symbol} color={item.color} /><div><strong>{item.symbol}</strong><small>{item.name}</small></div></div><span>{item.chain}</span><strong>{item.price}</strong><span className={item.change.startsWith('-') ? 'negative-value' : 'positive-value'}>{item.change}</span><span>{item.volume}</span><span>{item.cap}</span><span>{item.liquidity}</span><span>{item.holders}</span><button aria-label={`Open ${item.symbol}`}><ArrowUpRight size={18} /></button></div>)}</div><div className="table-footer"><span>{visible.length} reviewed assets</span><button className="text-button">Load more</button></div></Panel>
  </>
}
